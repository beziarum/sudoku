	     ...
	     do (loop for x2 from x1 below (+ 3 i)
		   do (loop for y2 from (if (= x1 x2) (+ 1 y1) j) below (+ 3 j) 
			 ...
			 do (loop for x3 from i below (+ 3 i)
			       do (loop for y3 from j below (+ 3 j)always (print (list (cons x1 y1)(cons x2 y2)(cons x3 y3)(aref grid x3 y3)(aref grid x2 y2)(or (/= x1 x2 x3) (/= y1 y2 y3))))
				     if (and (and (or (/= x1 x3)
						      (/= y1 y3))
						  (or (/= x2 x3)
						      (/= y2 y3)))
